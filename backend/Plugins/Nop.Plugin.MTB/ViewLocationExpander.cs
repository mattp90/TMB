    ...
    public class ViewLocationExpander : IViewLocationExpander
    {
            ...
            IEnumerable<string> viewLocations)
        {
            if (context.ActionContext.ActionDescriptor.DisplayName != null && !string.IsNullOrEmpty(context.AreaName) &&
                ...
                !context.ActionContext.ActionDescriptor.DisplayName.Contains("MTB"))
            {
                return viewLocations;
            }

            viewLocations = new[] { $"/Plugins/{MTB.PLUGIN_NAME_SPACE}/Views/{{0}}.cshtml" }.Concat(viewLocations);
            viewLocations = new[] { $"/Plugins/{MTB.PLUGIN_NAME_SPACE}/Views/{context.ControllerName}/{{0}}.cshtml" }.Concat(viewLocations);
